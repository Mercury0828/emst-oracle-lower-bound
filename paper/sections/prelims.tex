...
we realize it implicitly with the WSPD~\cite{CK95,HarPeled11},
...
\begin{tabular}{llll}
\toprule
random object & shared across? & concentration & failure handling\\
\midrule
leader-trial ranks (\cref{lem:leader}) & fresh per trial & variance $\le Mc$ & median-of-means\\
death-time ranks (\cref{sec:dt-sampler}) & fixed within a run & cond.\ on ranking & median-of-means\\
uniform seeds ($P$ or $Q$) & fresh per sample & i.i.d.\ given ranking & median-of-means\\
$K$-estimation batch (\cref{sec:support-oracle}) & fresh after fixing $h$ & negative-binomial & part of global budget\\
scale-test batches (\cref{lem:kwindow}) & one per scale & Chernoff, union over scales & part of global budget\\
global query budget (\cref{sec:wspd-cap}) & per call & Markov on total cost & abort $\to$ \textsc{fail}\\
median amplification (\cref{sec:main-combine}) & across runs & majority of $O(\log n)$ runs & \textsc{fail}=bad run\\
\bottomrule
\end{tabular}
\caption{The random objects and how each is concentrated and protected. Errors are combined by union
bounds, never by an assumed independence between estimators.}
\label{tab:random}
\end{table}
