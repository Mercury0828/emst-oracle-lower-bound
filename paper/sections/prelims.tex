...
The input is a multiset $P$ of $n$ points in the integer grid $[\Delta]^2$, where
$[\Delta]=\{0,1,\dots,\Delta-1\}$ and $\Delta=O(n)$; thus $\log\Delta=O(\log n)$. We access $P$ only
...
weighted graph $H$ on vertex set $V$ with a fixed root, assign independent uniform ranks to $V$ and
define the \emph{death time} $\tau(v)$ of a vertex as the bottleneck distance from $v$ to the root or to
any lower-ranked vertex. The multiset $\set{\tau(v):v\in V}$ equals $\set{0}\cup\set{\wt(e):e\in\MST(H)}$
