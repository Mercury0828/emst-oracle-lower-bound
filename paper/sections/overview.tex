...
This section is the intended entry point to the paper: it walks through the whole argument at the level of
ideas, giving a map for the formal development in \cref{sec:support,sec:dt,sec:main} and the appendix
proofs. It is self-contained and nothing in it is relied on later, so a reader who prefers to begin from
definitions may skip to \cref{sec:prelim} and refer back.
...
$\sqrt K\times\sqrt K$ block of points together with $s=\Theta(\sqrt K)$ far-flung satellites, each its
own cluster carrying a constant share of the MST weight. The satellites are an $s/K=\Theta(1/\sqrt K)$
fraction of the points, so a uniform point sample sees one only after $\Theta(K/s)=\Theta(\sqrt K)$ draws,
and a sample of $\poly\log$ many points misses them with high probability while they hold a constant
fraction of the weight. To resolve the cluster count by sampling \emph{points} one must therefore draw
...
The new primitive removes that barrier by sampling \emph{space} instead of mass. Fix a scale and lay
...
Second, an occupied exploration is cheap in expectation: the exploration visits the component in a
\emph{fixed order independent of the ranks}, and the $j$-th cell in that order is examined only when the
start has the smallest rank among the first $j-1$ cells, an event of probability $1/(j-1)$, so an
exploration touches $O(\log K)$ cells on average, the same harmonic bound that later controls the
death-time search.

\subsection{Two costs, one packing inequality, and the \texorpdfstring{$\sqrt K$}{sqrt K} bound}
\label{sec:ov-twopart}

The accounting that follows turns on five quantities, gathered here for reference.
\begin{center}\small
\begin{tabular}{@{}ll@{}}
  $M$ & size of the sampled cell universe (empty cells included)\\
  $U\ge c$ & a-priori upper bound on the component count $c=c_\Gamma(r)$\\
  $N$ & occupied cells a single exploration may touch\\
  $\alpha_r$ & additive accuracy target at scale $r$\\
  $b,\delta$ & active-cover parameters (packing inequality below)\\
\end{tabular}
\end{center}
...
can touch up to $N$ occupied cells; this is the \emph{exploration cost}. The point is that these two costs
scale with different quantities, $U$ (a bound on the component count) and $N$ (the occupied-cell count),
and we account for them separately.

It is worth dwelling on why separating the two costs is what saves a polynomial factor. If one bounded the
exploration cost the lazy way, by $N\le K$, and also let the trial count see $K$, every scale would cost
$\Theta(K)$ and the sum would be $\Theta(K)=\Theta(n^{2/3})$ at the balance point, worse than $\sqrt n$.
The gain comes from the asymmetry: the \emph{number of trials} is set by the variance $Mc$, which sees the
component count $c\le U$ and not the occupied-cell count, so a scale with few components but many cells is
cheap to estimate; the \emph{work per trial} sees $N$, but a trial does that work only when its cell is
occupied, an event of probability $N/M$, so the two $N$'s and $M$'s partly cancel. Bounding the first sum
by $U$ and the second by $N\le K$, and summing each against the packing inequality, leaves $\sqrt K$ in
both.
...
\Cref{lem:support} estimates a grid support, not the input $P$. Two gaps separate them: the support has
$K_0=\Theta(n^{2/3})$ cells while $P$ has $n$ points, and the support is a snapped, coarsened version of
$P$. Bridging the first cheaply is why we cannot simply run the support estimator on $P$ directly (that
would have $K=n$ and cost $\Otil(\sqrt n)$); bridging the second is why a controlled distortion argument is
needed. Three steps do it, and they are the substance of \cref{sec:dt}. Fix a guess $G\ge W$ and a clip
level $L=G/K_0$ with $K_0=\ceil{n^{2/3}}$, and split the MST weight at $L$ into a \emph{bulk} $A_L=\sum_e
\min\{\abs e,L\}$ (the short edges, of which there are many) and a \emph{tail} $B_L=\sum_e(\abs e-L)_+$
(the few long ones that carry the heavy structure). The split is chosen so that each part is handled by
the estimator suited to it.
...
$\Theta_\xi(K_0)$ cells (\cref{lem:kwindow}), and snap each point to its cell center. Snapping moves points by
...
\emph{Assembly.} The identity behind the algorithm is $W=A_L(P)+B_L(P)$ with $B_L(P)\approx B_L(Q)=
\Wq-A_L(Q)$, so $W\approx A_L(P)+\Wq-A_L(Q)$. We therefore add the bulk of $P$ to the tail of the snapped
support,
\begin{equation}\label{eq:ov-assembly}
  \widehat W=\widehat A_P+\widehat\Wq-\widehat A_Q ,
\end{equation}
in which $\widehat\Wq$ carries the heavy tail (its whole reason for being computed by the support
estimator) and the two death-time terms supply and then cancel the bulk that the support and the input
share. Choosing the internal accuracy $\xi=\Theta(\eps)$ makes every error term $O(\eps G)$, and since all
four are measured against the same guess scale $G$ they add to $O(\eps G)$. The remaining issue is that the
algorithm needs a guess $G\ge W$ to fix $L$ and the budgets, while $W$ is the unknown. This is handled by a
geometric search: the per-guess output is accurate whenever $G\ge W$, so sweeping $G$ downward by halving
and stopping at the first $G$ with $\widehat W(G)\ge G/8$ brackets $G_0\in[W,16W)$, after which a single
calibrated call on $G_0$ delivers the $(1\pm\eps)$ estimate. Each of the three errors is bounded with
constant probability and the three are combined by a union bound, so no independence between them is
needed; an aborted call (the global budget is a hard cap) is treated as a failed run inside the median
amplification that protects all $O(\log n)$ guesses at once.
...
using only range counts. We build it from the WSPD of the quadtree on
...
\cref{sec:wspd}; it is the step inherited from~\cite{DMOSW25}, adapted to the local family of
\cref{sec:wspd-spanner}.
...
\subsection{What is new, against the \texorpdfstring{$\Otil(\sqrt n)$}{Otil(sqrt n)} approach}
\label{sec:ov-compare}

Our algorithm shares its scaffolding with Driemel et al.~\cite{DMOSW25} -- the cluster-count reduction, the
WSPD spanner, range-count navigation -- and differs at a single step: estimating a component count at one
scale. Their subroutine samples vertices and explores components, paying $\Theta(\sqrt K)$ \emph{per scale}
just to see the heavy outliers (a vanishing fraction of the mass), which sums to $\Theta(\sqrt n)$. The
empty-cell leader estimator samples cells from a universe including empty ones, so an outlier is one cell
among $M$ found with probability $1/M$ regardless of its mass; its variance sees the component count, and
the packing inequality collapses the per-scale costs to $\Otil(\sqrt K)$ \emph{total}. Balancing against
the $n/K$ cost of learning $K$ moves the rate to $\Otil(n^{1/3})$, with the leader estimator the step that
yields the polynomial improvement.
