...
calibrated call on $G_0$ delivers the $(1\pm\eps)$ estimate. The three estimators draw fresh samples, so
their errors are independent, and no seed is reused across thresholds or across the three pieces; an
aborted call (the global budget is a hard cap) is treated as a failed run inside the median amplification
that protects all $O(\log n)$ guesses at once.
